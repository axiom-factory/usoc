...
def extract_raw_signals(obj):
    """
    Recursively drills through Components, PureInterfaces, and signature bundles
    to extract only the base Signal primitives for the Verilog compiler.
    """
    signals = []
    
    # Base Case: It's an isolated Signal primitive
    if isinstance(obj, Signal):
        signals.append(obj)
        
    # Recursive Case: It's an interface, flipped view, or structural component
    elif hasattr(obj, "signature") and hasattr(obj.signature, "members"):
        for member_name in obj.signature.members.keys():
            # Recursively pull signals from the child attributes
...
def build_ecp5(top, target_dir=Path("target")):
    """
    Synthesizes and routes a component targeting an industrial ECP5-5G LFE5UM5G-85F chip.
    """
    name = top.__class__.__name__
    build_dir = target_dir / "ecp5" / name
    build_dir.mkdir(parents=True, exist_ok=True)

    # Write a clean, dynamic LPF file targeting a fast 200 MHz system clock profile
    lpf_path = build_dir / f"{name}.lpf"
    with open(lpf_path, "w") as f:
        f.write('FREQUENCY NET "clk" 200.00 MHz;\n')
...
def build_ice40(top, target_dir=Path("target")):
    """
    Synthesizes and routes a component targeting a cheap, ultra-low-power iCE40 HX8K chip.
    """
    name = top.__class__.__name__
    build_dir = target_dir / "ice40" / name
    build_dir.mkdir(parents=True, exist_ok=True)

    # Write a clean PCF file constraining the clock pin for iCE40
    pcf_path = build_dir / f"{name}.pcf"
    with open(pcf_path, "w") as f:
        # We define a 100 MHz constraint target to force nextpnr to search for max fclk limits
        f.write("set_frequency clk 100.00\n")
